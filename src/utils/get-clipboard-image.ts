...
const readClipboardScript = (
    ...
    const buffer = fs.readFileSync(AppContext.extContext.asAbsolutePath(`dist/assets/scripts/clipboard/${scriptName}`));
...
const getClipboardImage = async (): Promise<IClipboardImage> => {
    const imagePath = path.join(
        AppContext.extContext.asAbsolutePath('./'),
        ...
        execution.stdout.on('data', (data: Buffer) => {
            if (platform === 'linux') {
                if (data.toString().trim() === 'no xclip') {
                    window.showWarningMessage('xclip not found, Please install xclip first');
